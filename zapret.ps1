...
$ARGS += "--filter-udp=443 --hostlist=`"$folderPath\google.txt`" --dpi-desync=fake --dpi-desync-repeats=11 --dpi-desync-fake-quic=`"$folderPath\quic.txt`" --new "
$ARGS += "--filter-udp=80 --hostlist=`"$folderPath\google.txt`" --dpi-desync=fake --dpi-desync-repeats=11 --dpi-desync-fake-quic=`"$folderPath\quic.txt`" --new "
...
foreach ($process in $processesToKill) {
    Stop-Process -Name $process -Force -ErrorAction Stop | Out-Null SilentlyContinue
}
